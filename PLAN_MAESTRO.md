...
> Documento guía para desarrollar con **Claude Code**.
> Colócalo en la raíz del proyecto y renómbralo a `CLAUDE.md` si quieres que Claude Code lo tome como contexto permanente.
...
## 10. Cómo trabajar esto con Claude Code

1. Coloca este archivo como `CLAUDE.md` en la raíz del proyecto (junto a `generar_publicacion.py`, `logo-sin-fondo.png`, `fonts/` y tus fotos).
2. Inicializa git (`git init`) para poder deshacer a nivel de proyecto y revisar diffs.
3. Pídele a Claude Code **una tarea de la Fase 1 a la vez** (1.1, luego 1.2, …), y que haga commit al cerrar cada una.
