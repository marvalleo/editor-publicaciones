# Fase 1 · Tarea 1.4 (Ronda A) — Conectar el modelo + selección con resaltado Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Conectar por fin `dcpub/app.py` con el modelo formal `Project`/`Slide`/`Layer` (`dcpub/models.py`, Tarea 1.2) en vez del diccionario `self.el`, y agregar selección de capas por click con resaltado visual, respetando `locked`, más una lectura en pantalla de coordenadas/tamaño de la capa seleccionada. Esta es la primera de dos rondas de la Tarea 1.4 del plan maestro — handles de resize, movimiento por teclado y snap con guías quedan para la Ronda B, una vez que esta esté mergeada y probada.

**Architecture:** `App` pasa a tener `self.project`/`self.slide` (creados con `crear_proyecto_por_defecto()`) en vez de `self.el`. Como el vocabulario de tipos del modelo (`"photo"|"logo"|"text"|"box"`, con `role` para distinguir título/subtítulo) no coincide con el vocabulario que ya espera `render.py` (`"photo"|"logo"|"title"|"sub"|"desc"`, sin cambios en esta tarea), se agrega un pequeño adaptador (`_layer_by_kind`, `_build_layers`) que traduce entre ambos — así `dcpub/render.py` no se toca para nada. El contenido de texto (título/subtítulo/descripción/ícono) y la ruta de la foto siguen viniendo de los widgets de Tkinter existentes (como hasta ahora); solo la geometría (x, y, tamaño/w/h, zoom, offset) pasa a vivir en los objetos `Layer` del modelo, porque es lo que la selección y los futuros handles (Ronda B) necesitan mutar directamente.
...
- Esta ronda **no** modifica `dcpub/render.py` ni `dcpub/models.py` — solo `dcpub/app.py`.
- El contenido de texto/ícono/ruta de foto sigue viniendo de los widgets de Tkinter (`self.txt_title`, `self.v_sub`, `self.txt_desc`, `self.v_icon`, `self.v_photo`), **no** de los campos `text`/`icon`/`src` del modelo — esos campos existen en el modelo para cuando la Tarea 1.8 (guardar/cargar proyecto) los necesite, pero todavía no son la fuente de verdad. Solo la geometría (`x`, `y`, `w`, `h`, `size`, `zoom`, `offset_x`, `offset_y`) vive en el modelo desde ahora.
- Una capa con `locked=True` no se puede seleccionar ni mover (ya lo exige la Tarea 1.4 del plan maestro).
- Una capa con `visible=False` no debe dibujarse — se resuelve excluyéndola de la lista que arma `_build_layers()`, sin tocar `render.py`.
- No se extrae `CanvasEditor` a un módulo propio todavía (decisión confirmada con el usuario) — todo sigue en `dcpub/app.py`.
- Nombres y comentarios en español; sin pseudocódigo ni partes "por completar".
- `dcpub/app.py` no tiene test automatizado (GUI sin display en este entorno) — verificación por `py_compile` + `ast.parse` + suite completa de tests + revisión manual del usuario, mismo patrón que las tareas anteriores.
- Commit al cerrar cada tarea (task) de este plan.

---

### Task 1: Conectar `Project`/`Slide`/`Layer` en `App` (sin cambiar comportamiento visible)

**Files:**
- Modify: `dcpub/app.py` (reemplazo completo)

**Interfaces:**
- Consumes: `dcpub.models.crear_proyecto_por_defecto(photo_path: str = "") -> Project` (Tarea 1.2, ya existe). `dcpub.constants.FORMATOS` (Tarea 1.3, ya existe).
- Produces: `App.project: Project`, `App.slide: Slide` (reemplazan a `App.el`/`App.format`). `App._layer_by_kind(kind: str) -> Layer | None` traduce entre el vocabulario del render (`"photo"|"logo"|"title"|"sub"|"desc"`) y los tipos/roles reales del modelo. `App._get_layer_value(elem, param)` / `App._set_layer_value(elem, param, value)` leen/escriben atributos de los objetos `Layer` (con el caso especial de que el "tamaño" del logo vive en `Layer.w`/`Layer.h`, no en un campo `size`). El comportamiento visible de la app (sliders, drag con mouse, formato, reset) debe ser **idéntico** al de antes de esta tarea.
...
Expected: `OK` — los 39 tests existentes (fonts, render, models, constants) no dependen de `app.py`, así que deben seguir pasando sin cambios; esto confirma que no se rompió nada en `dcpub/models.py`, `dcpub/constants.py` ni `dcpub/render.py` (que esta tarea no toca).

- [ ] **Step 4: Commit**

```bash
git add dcpub/app.py
git commit -m "Conectar app.py con el modelo Project/Slide/Layer de dcpub.models"
```

---

### Task 2: Selección por click + resaltado + respeto de locked + lectura de coordenadas

**Files:**
- Modify: `dcpub/app.py` (reemplazo completo)

**Interfaces:**
- Consumes: `Layer.locked`, `Layer.visible` (Tarea 1.2, ya existen). Todo lo de Task 1.
- Produces: `App._selected: Layer | None` (capa actualmente seleccionada). `App._kind_of(layer) -> str | None` (inverso de `_layer_by_kind`). `App._set_selected(layer)`. `App.v_readout: tk.StringVar` con las coordenadas/tamaño de la capa seleccionada, visible en pantalla.
...
Run: `python -m unittest discover -s tests -v`
Expected: `OK` (39 tests, sin cambios respecto a Task 1 — ninguno de ellos ejercita `app.py` directamente).

- [ ] **Step 4: Commit**

```bash
git add dcpub/app.py
git commit -m "Agregar seleccion de capas por click con resaltado y lectura de coordenadas"
```
...
- **Cobertura de esta ronda:** de los criterios de aceptación de la Tarea 1.4 en CLAUDE.md, esta ronda cubre "click selecciona la capa superior bajo el cursor", "bounding box visible en la capa seleccionada", "mostrar en pantalla coordenadas y tamaño de la capa seleccionada" y "respeta locked". Quedan para la Ronda B: los 8 handles de resize, el movimiento por teclado, y el snap con guías — están fuera de alcance de este plan, no son un olvido.
- **Sin placeholders:** cada paso trae el contenido completo y ejecutable de `dcpub/app.py` en su estado acumulado.
- **Consistencia de tipos:** `_layer_by_kind`/`_get_layer_value`/`_set_layer_value` (Task 1) se usan igual en `_on_slider`, `_sync_sliders`, `_reset`, `_on_drag` (Task 1) y en `_kind_of`, `_update_readout`, `_draw_selection_overlay`, `_on_press` (Task 2).
- **Riesgo conocido:** igual que en tareas anteriores, `dcpub/app.py` no tiene test automatizado (GUI sin display). El riesgo específico de esta ronda es que Task 1 es un refactor grande (cambia la fuente de verdad de la geometría de un dict a objetos `Layer`) sin ningún cambio de comportamiento visible — la verificación real de que sigue funcionando exactamente igual que antes depende de la revisión manual del usuario al final.
- **Decisión de alcance:** el contenido de texto/ícono/ruta de foto sigue viniendo de los widgets de Tkinter, no de los campos `text`/`icon`/`src` del modelo (confirmado en el diseño). `Layer.src` de la foto sí se mantiene sincronizado en `_browse` por prolijidad hacia la Tarea 1.8, aunque `_build_layers()` no lo lea todavía.
